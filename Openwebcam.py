...
import keyboard
import time # To help with debounce (preventing rapid-fire key presses)
...
mp_draw = mp.solutions.drawing_utils # Utility for drawing the hand skeleton
...
# Get frame dimensions for setting control zones
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

# Define control zones (20% from each edge)
zone_size = int(frame_width * 0.20)
LEFT_ZONE_MAX_X = zone_size
RIGHT_ZONE_MIN_X = frame_width - zone_size

# Debounce timer to prevent multiple actions from one quick gesture
last_action_time = 0
...
while True:
    ...
    # Flip the frame for a mirror view, making control intuitive
    frame = cv2.flip(frame, 1)

    # Convert the BGR image to RGB before processing (MediaPipe requirement)
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # --- Draw the hand skeleton on the frame (optional, but helpful) ---
            mp_draw.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)
            
            # --- GET INDEX FINGER TIP POSITION (Landmark #8) ---
            # Get the coordinates of the index fingertip (landmark 8)
            # The coordinates are normalized (0 to 1), so we multiply by frame dimensions
            ...
            # --- GESTURE CONTROL LOGIC (with Debounce) ---
            
            # 1. Check for LEFT zone (PREVIOUS action)
            ...
            # 2. Check for RIGHT zone (NEXT action)
    ...
    # --- VISUAL FEEDBACK (Draw the control zones) ---
